...
(cat $4/acceptable_header.txt | grep -v '#CHROM'; zcat $6/$3.vcf.gz | grep '#CHROM' | head -1; zcat $6/$3.vcf.gz | grep -v '#' | awk 'BEGIN {FS = "\t"} {$7 = "PASS"; $8 = "."; gsub(":.*", "", $9); gsub(":.*", "", $10); gsub("\\|", "/", $10); print "chr" $1 "\t" $2 "\t" $3 "\t" $4 "\t" $5 "\t" $6 "\t" $7 "\t" $8 "\t" $9 "\t" $10}') | gzip > $6/${3}_fixed.vcf.gz
